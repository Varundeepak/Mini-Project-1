...
public class NotifyUser {
    public void noti()
    {
        ...
        query="select issue.memberid,member.contact from member,issue where member.memberid=issue.memberid and issue.dateofnotify=(select current_date());";
        rs=stmt.executeQuery(query);
        while(rs.next())
        { int ijk=rs.getInt("issue.memberid");
            String abc=rs.getString(1);
        String message = "Greetings Member. This message is to remind you that the due date for the book you borrowed is in 2 days. Kindly remember to return the book in time. Thank You!!!";		
		String phone = abc;
		...
		bufferedReader.close();
            String query3="update issue set dateofnotify=null where memberid="+ijk+";";
